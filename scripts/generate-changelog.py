...
def format_commit(commit: Dict[str, str], include_hash: bool = True, include_author: bool = False) -> str:
    """格式化单个commit"""
    subject = commit['subject']
    
    # 移除conventional commit前缀以获得更清晰的描述
    clean_subject = re.sub(r'^(feat|fix|docs?|style|refactor|perf|test|build|ci|chore)(\(.+\))?:\s*', '', subject, flags=re.IGNORECASE)
    
    parts = [f"- {clean_subject}"]
    
    if include_hash:
        short_hash = commit['hash'][:7]
        parts.append(f" ([{short_hash}](https://github.com/mudssky/pyutils/commit/{commit['hash']}))") 
...
def generate_changelog_section(title: str, emoji: str, commits: List[Dict[str, str]], 
                              include_hash: bool = True, include_author: bool = False) -> List[str]:
    """生成changelog的一个部分"""
    if not commits:
        return []
...
def generate_changelog(from_tag: Optional[str] = None, to_tag: str = "HEAD", 
    ...
    for title, emoji, commits in sections:
        lines.extend(generate_changelog_section(title, emoji, commits, include_hash, include_author))
    ...
    if total_commits > 0:
        ...
        if from_tag and to_tag == "HEAD":
            lines.append(f"**Compare**: https://github.com/mudssky/pyutils/compare/{from_tag}...HEAD")
        elif from_tag and to_tag != "HEAD":
            lines.append(f"**Compare**: https://github.com/mudssky/pyutils/compare/{from_tag}...{to_tag}")
...
def generate_full_changelog(include_hash: bool = True, include_author: bool = False) -> str:
    """生成完整的changelog"""
    tags = get_tags()
    ...
    if unreleased_commits:
        ...
        lines.append("")
        unreleased_changelog = generate_changelog(tags[0], "HEAD", include_hash, include_author)
        # 移除标题行
    ...
    for i, tag in enumerate(tags):
        ...
        tag_changelog = generate_changelog(from_tag, tag, include_hash, include_author, version)
...
def main():
    ...
    parser.add_argument("--from", dest="from_tag", help="Generate changelog from this tag/commit")
    parser.add_argument("--to", dest="to_tag", default="HEAD", help="Generate changelog to this tag/commit (default: HEAD)")
    parser.add_argument("--all", action="store_true", help="Generate full changelog for all releases")
    parser.add_argument("--output", "-o", help="Output file (default: stdout)")
    parser.add_argument("--no-hash", action="store_true", help="Don't include commit hashes")
    parser.add_argument("--include-author", action="store_true", help="Include commit authors")
