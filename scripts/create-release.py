...
def update_version_files(version: str, dry_run: bool = False) -> None:
    ...
    for file_path, pattern, replacement in files_to_update:
        ...
        if content != new_content:
            ...
            print(f"i  No changes needed in {file_path}")
...
def create_tag_and_push(
    version: str, dry_run: bool = False, push: bool = False
) -> None:
    """创建tag并推送."""
    ...
    try:
        ...
        if push:
            ...
            print("i  Tag created locally. Use --push to push to origin")
            print(f"i  Or run: git push origin {tag_name}")
...
def main():
    ...
    version_group.add_argument(
        "--version", help="Specific version to release (e.g., 1.0.0)"
    )
    version_group.add_argument(
    ...
    parser.add_argument(
        "--push", action="store_true", help="Push the tag to origin (triggers CI/CD)"
    )
    parser.add_argument(
        "--skip-checks", action="store_true", help="Skip git status and branch checks"
    )
    ...
    if current_version != new_version:
        ...
        if not args.dry_run:
            ...
            print("✅ Version updated and committed")
    ...
    if not args.dry_run:
        ...
        if args.push:
            print(
                ...
                "https://github.com/mudssky/pyutils/actions"
            )
            print(
                f"📦 PyPI package will be available at: "
                f"https://pypi.org/project/mudssky-pyutils/{new_version}/"
            )
        else:
            print(
                f"i  To trigger the release, push the tag: "
