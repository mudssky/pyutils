...
def parse_url(url: str, base: Optional[str] = None) -> Dict[str, str]:
...
def decode_uri_component(text: str) -> str:
    ...
    if re.search(r'%[^0-9A-Fa-f]', text) or re.search(r'%[0-9A-Fa-f][^0-9A-Fa-f]', text):
...
def build_url(protocol: str = "http", hostname: str = "", port: Optional[int] = None, 
              pathname: str = "", query_params: Optional[Dict[str, str]] = None, 
              hash_fragment: str = "") -> str:
    ...
    Examples:
        >>> build_url(hostname='example.com', pathname='/api/users', query_params={'page': '1'})
...
def get_query_params(url: str) -> Dict[str, str]:
    ...
    Examples:
        >>> get_query_params('https://example.com/path?foo=bar&baz=qux')
        {'foo': 'bar', 'baz': 'qux'}
